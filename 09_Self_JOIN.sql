...
two services that link these stops have a count of 2.
...
FROM
  (SELECT stopb.id, stopb.name, a.company, a.num
...
JOIN
  (SELECT stopb.id, stopb.name, a.company, a.num
    FROM route a JOIN route b ON (a.company=b.company AND a.num=b.num)
         JOIN stops stopa ON (a.stop=stopa.id)
         JOIN stops stopb ON (b.stop=stopb.id)
    WHERE stopa.name='Sighthill') d
ON c.id = d.id
ORDER BY CAST(c.num as unsigned), name;
